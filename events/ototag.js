...
module.exports = (oldUser, newUser) => {
  ...
  if(!user) return;
  const embed = new discord.MessageEmbed().setAuthor(user.displayName, user.user.avatarURL({dynamic: true})).setFooter("Gruzz").setColor(client.randomColor()).setTimestamp();
  ...
  if(newUser.username.includes(ayarlar.tag) && !user.roles.cache.has(ayarlar.ekipRolu)){
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} kişisi  \`${ayarlar.tag}\` tagını alarak <@&${ayarlar.ekipRolu}> ekibimize katıldı!`).setColor("RANDOM")).catch();
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} kişisi  \`${ayarlar.tag}\` tagını çıkararak <@&${ayarlar.ekipRolu}> ekibimizden ayrıldı!`).setColor("RANDOM")).catch();
  }
}
