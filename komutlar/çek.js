...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  ...
  if (!message.member.voice.channel || !uye.voice.channel || message.member.voice.channelID == uye.voice.channelID) return message.channel.send(embed.setDescription("Siz veya belirtilen üye ses kanalında değil!")).then(x => x.delete({timeout: 5000}));
  if (message.member.hasPermission("ADMINISTRATOR")) {
    ...
    message.channel.send(`${uye}`, {embed: embed.setAuthor(uye.displayName, uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${message.author} seni ses kanalına çekmek istiyor. Onaylıyor musun?`)}).then(async msj => {
      await msj.react('✅');
